...
function tampilkanDataDiTabel(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        cells[0].innerHTML = `<img src="${data[i].foto}" alt="${data[i].nama}" style="max-width: 100px;">`;
        cells[1].innerHTML = data[i].nama;
        cells[2].innerHTML = data[i].penjelasan;
        cells[3].innerHTML = data[i].jenis;
        cells[4].innerHTML = data[i].rating;

        cells[5].innerHTML = `<button class="hapusDataButton" data-id="${data[i].id}">Hapus</button>`;
        cells[5].querySelector('.hapusDataButton').addEventListener('click', function (event) {
...
function validasiForm() {
    ...
    if (nama.trim() === "" || penjelasan.trim() === "" || jenis.trim() === "") {
